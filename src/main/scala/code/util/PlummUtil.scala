...
object PlummUtil {
  ...
  def generateDES(string: String) : String = {
    val des = MessageDigest.getInstance("DES");
    des.update(string.getBytes())

    val digest = des.digest();
    //return new BASE64Encoder().encode(digest)
    return new String(Base64.encodeBase64(digest))
  }
  ...
  def generateSHA1_php(string: String) : String = {
    ...
    //generateSHA1(string)
    val sha_string = DigestUtils.sha1Hex(string)
