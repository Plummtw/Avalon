...
class UserEntryRegisterSnippet extends StatefulSnippet with Logger {
  // 參數
  private var uname      = ""
  ...
  def render = {
    ...
    def process() : Unit = {
      ...
      val userentry = UserEntry.create.uname(uname.trim()).handle_name(handle_name.replace('　',' ').trim()).sex(sex)
                                ...
                                .role_flags(role).subrole(align).last_words("")
      ...
      userentry.validate match {
        case Nil => ;
        case xs  => S.error(xs)
                    S.redirectTo("room_register.html?&room_no=" + room_id)
      }
    ...
    "#wish-align"      #> (if (room.has_flag(RoomFlagEnum.WISH_ALIGN))
                             SHtml.select(Seq(("","不指定"),("V","村民"),("W","人狼")),
                              Full(align),  x => align = x) 
                           else
                             SHtml.select(Seq(("","不指定")),
                              Full(align),  x => align = x)) &
    "#wish-role"      #> (if (room.has_flag(RoomFlagEnum.WISH_ROLE))
                             SHtml.select(RoleEnum.WISH_ROLE_LIST.map(x=>(x.toString, RoleEnum.get_role(x).role_name)),
                              Full(role),  x => role = x)
                           else
                             SHtml.select(Seq(("","不指定")),
                              Full(role),  x => role = x)) &
...
class UserEntryLoginSnippet extends Logger {
    ...
    val userentry_box : Box[UserEntry] =
      if ((param_uname != "") && (param_password != "")) {
        UserEntry.find(By(UserEntry.uname, param_uname), 
          ...
          case Full(userentry_loaded) if (!userentry_loaded.revoked.is) => warn("Login with uname/password : " + userentry_loaded.id.is)
                                                                           CurrentRoom.set(Box !! room)
                                                                           Room_R.set(room)
  ...
  def logout(room: Room) = {
    ...
    CurrentUserEntry.get match {
      case Full(x) =>  warn("CurrentUserEntry.set null, previous : " + x.id.is)
      case _       =>  warn("CurrentUserEntry.set null ")
    }
    ...
    if (CurrentUserEntry_R.get != null)
      warn("CurrentUserEntry_R.set null, previous : " + CurrentUserEntry_R.get.id.is)
    else
      warn("CurrentUserEntry_R.set null ")
    ...
    S.session.openOrThrowException("Session : Throw Exception").terminateHint 
